...
function Register() {
  ...
  const handleSubmit = (e) => {
    ...
    if (!validEmail(trimEmail)) { setError("Enter a valid email."); return; }
    if (!validPassword(password)) { setError("Password: 8+ chars, 1 capital, 1 small, 1 number, 1 symbol."); return; }
    if (password !== confirm) { setError("Passwords do not match."); return; }

    const result = register(name.trim(), trimEmail, password);
    if (result.ok) {
